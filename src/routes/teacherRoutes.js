...
const createTeacherValidation = [
  ...
  body("branch_ids")
    .isArray({ min: 1 })
    .withMessage("At least one branch_id is required"),
  body("branch_ids.*")
    .isInt({ min: 1 })
    .withMessage("Each branch_id must be a positive integer"),

  body("division_ids")
    .isArray({ min: 1 })
    .withMessage("At least one division_id is required"),
  body("division_ids.*")
    .isInt({ min: 1 })
    .withMessage("Each division_id must be a positive integer"),
];
...
const updateTeacherValidation = [
  ...
  body("division_ids")
    .optional()
    .isArray({ min: 1 })
    .withMessage("division_ids must be a non-empty array"),
  body("division_ids.*")
    .optional()
    .isInt({ min: 1 })
    .withMessage("Each division_id must be a positive integer"),
...
const migrateTeacherValidation = [
...
router.post(
  "/:id/migrate",
  migrateTeacherValidation,
  TeacherController.migrate,
);
