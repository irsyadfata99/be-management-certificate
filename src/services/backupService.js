...
if (!fs.existsSync(BACKUP_DIR_RESOLVED)) {
  fs.mkdirSync(BACKUP_DIR_RESOLVED, { recursive: true });
...
class BackupService {
  ...
  static async createBackup(adminId, description = null) {
    ...
    // Validate generated path is still inside BACKUP_DIR (safeguard against
    // unexpected date/description edge cases)
    _assertPathIsInsideBackupDir(filePath);
